...
3. **Fix loop (max 2 rounds).** Both `PASS` → return `DONE` (carry any `GATE REQUIRED` lines from the verifier's final pass into `GATES` — the orchestrator puts them in the PR body). If the reviewer returned `CHANGES_REQUIRED` and/or the verifier returned `FAIL`: spawn `wf-executor` **once** with the combined list (reviewer BLOCKING items + verifier FAILURES, + CONTEXT_PACK, MANUAL), then re-run step 2. The reviewer reports coverage-first — its NON_BLOCKING list may be long and include low-confidence suspicions; **never** pass NON_BLOCKING items to the executor, regardless of count — a fix invalidates both verdicts, so always re-run both together. After 2 fix rounds with anything still blocking/failing → return `NEEDS_HUMAN` (stage `review`, `verify`, or `review+verify` per what remains) with the remaining items.
